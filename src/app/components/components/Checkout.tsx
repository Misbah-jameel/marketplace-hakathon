...
export default function CheckoutForm({ amount, currency = "usd" }: Props) {
  ...
  useEffect(() => {
    const fetchClientSecret = async () => {
      try {
        ...
        console.log("Fetching client secret:", { amount: amountInSubunits, currency })
        ...
        if (!response.ok || !data.clientSecret) {
          console.error("Error fetching client secret:", data.error || "Unknown error")
        ...
        setErrorMessage(err instanceof Error ? err.message : "Payment failed to initialize")
      }
    }
  ...
  if (!clientSecret) {
    return (
      <div className="flex flex-col items-center justify-center h-64">
        {errorMessage ? (
          <div className="text-red-600 p-4 bg-red-50 rounded-lg">{errorMessage}</div>
        ) : (
          <>
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto">
      ...
      {errorMessage && <div className="mt-4 p-3 bg-red-50 text-red-700 rounded-lg">{errorMessage}</div>}
    </form>
  )
}
